...
void map()
{
  ...
  u8g2.drawXBM(executorPosX, 38, 11, 7, image_Layer_5_bits);
  ...
  u8g2.drawXBM(shipPosX+8, 22, 4, 3, image_Layer_21_bits);
  u8g2.drawXBM(shipPosX-4, 22, 4, 3, image_Layer_21_bits);
  u8g2.drawXBM(shipPosX, 26, 4, 3, image_Layer_21_bits);
  u8g2.sendBuffer();
}
...
void executeOrder()
{
  ...
  // u8g2.drawStr(4, 11, "ALL COMMANDS ALERTED");
  u8g2.drawStr(1, 8, "TRANSMISSION COMPLETE");
  u8g2.drawStr(7, 64, "GLORY TO THE EMPIRE");
  u8g2.drawXBM(56, 24, 15, 15, image_Layer_6_bits);
  u8g2.sendBuffer();
}
...
{
  u8g2.clearDisplay();
  u8g2.drawXBM(59, 24, 11, 16, image_cross_contour_bits);
  u8g2.sendBuffer();
}
...
void loop()
{
  ...
  if (!selectionActive && !mp3->isRunning())
  {
  map();
  if (executorPosX < 0)
    executorPosX = 131;
